...
<?php 
	if(isset($_GET['tag'])) {
		...
		$q = "SELECT * FROM cms_posts where post_tags like '%$tag%'";
		...
		// find posts that match tags search
		...
		if(!$posts) {	
			...
			if($post_count == 0) {
				$page_count = 0;
				$div_class = 'danger';
				$div_msg = "Sorry, no posts found for search on <strong>'$tag'</strong>.";
			} else {
				$page_count = ceil($post_count / 8);
				$div_class = 'success';
				$div_msg = "Showing published posts for search word <strong>'$tag'</strong>.";
...
<div class="pagination-div">
	<ul class="pagination pagination-sm"  >
		<li>
				<a href="sposts.php?p=1&tag=<?php echo $tag;?>" aria-label="Previous">
	  ...
	  <?php if($i == $page):?>
	  <li class="active">
		  	<a href="sposts.php?p=<?php echo $i;?>&tag=<?php echo $tag;?>"><?php echo $i;?></a>
	  	</li>
	  	<?php else:?>
	  <li>
		  	<a href="sposts.php?p=<?php echo $i;?>&tag=<?php echo $tag;?>"><?php echo $i;?></a>
	  ...
	  <?php endfor;?>
	  <li>
			<a href="sposts.php?p=<?php echo $total_pages;?>&tag=<?php echo $tag;?>" 
				aria-label="Next">
